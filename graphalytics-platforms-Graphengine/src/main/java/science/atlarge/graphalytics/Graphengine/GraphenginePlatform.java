 ...
 */
package science.atlarge.graphalytics.Graphengine;
...
import science.atlarge.graphalytics.Graphengine.algorithms.bfs.BreadthFirstSearchJob;
import science.atlarge.graphalytics.Graphengine.algorithms.cdlp.CommunityDetectionLPJob;
import science.atlarge.graphalytics.Graphengine.algorithms.lcc.LocalClusteringCoefficientJob;
import science.atlarge.graphalytics.Graphengine.algorithms.pr.PageRankJob;
import science.atlarge.graphalytics.Graphengine.algorithms.sssp.SingleSourceShortestPathsJob;
import science.atlarge.graphalytics.Graphengine.algorithms.wcc.WeaklyConnectedComponentsJob;
import java.nio.file.Paths;
import java.nio.file.Path;

/**
 * Graphengine platform driver for the Graphalytics benchmark.
 *
 * @author Julius Kliss
 */
public class GraphenginePlatform implements Platform {

	protected static final Logger LOG = LogManager.getLogger();

	public static final String PLATFORM_NAME = "Graphengine";
	public GraphengineLoader loader;

	public GraphenginePlatform() {
	...
	public LoadedGraph loadGraph(FormattedGraph formattedGraph) throws Exception {
		GraphengineConfiguration platformConfig = GraphengineConfiguration.parsePropertiesFile();
		loader = new GraphengineLoader(formattedGraph, platformConfig);
		...
		try {
			...
			if (exitCode != 0) {
				throw new PlatformExecutionException("Graphengine exited with an error code: " + exitCode);
			}
		} catch (Exception e) {
			throw new PlatformExecutionException("Failed to load a Graphengine dataset.", e);
		}
	...
	public void deleteGraph(LoadedGraph loadedGraph) throws Exception {
		...
		try {
			...
			throw new PlatformExecutionException("Failed to unload a Graphengine dataset.", e);
		}
	...
	public void startup(RunSpecification runSpecification) throws Exception {
		...
		GraphengineCollector.startPlatformLogging(logDir);
	}
	...
	public void run(RunSpecification runSpecification) throws PlatformExecutionException {
		...
		try {
			...
			throw new PlatformExecutionException("Failed to execute a Graphengine job.", e);
		}
	...
	public BenchmarkMetrics finalize(RunSpecification runSpecification) throws Exception {
		GraphengineCollector.stopPlatformLogging();
		BenchmarkRunSetup benchmarkRunSetup = runSpecification.getBenchmarkRunSetup();
		Path logDir = benchmarkRunSetup.getLogDir().resolve("platform");

		System.out.println("###" + logDir.toString());
		LOG.info("###" + logDir.toString());

		BenchmarkMetrics metrics = new BenchmarkMetrics();
		metrics.setProcessingTime(GraphengineCollector.collectProcessingTime(logDir));
		return metrics;
	}
